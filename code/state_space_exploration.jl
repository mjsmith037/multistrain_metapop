...
# to use in parallel settings, un-comment 'addprocs' (editing to match number of cores desired)
# addprocs(1)
...
@everywhere begin
    ...
    function count_unique_local_minima(vector, digits=nothing)
        ...
        # now check for negative monotonicity in cyclical dynamics => assume going extinct
        if all(((minima - ShiftedArrays.lag(minima)) .<= 0)[2:end]) return(-1) end
        ...
        if all(((minima - ShiftedArrays.lag(minima)) .>= 0)[2:end])
            ...
            if all(((maxima - ShiftedArrays.lag(maxima)) .<= 0)[2:end]) return(0) else return(1) end
        end
...
function state_space_exploration(;strainstruct = [2 2],
    ...
    results = pmap(x -> clean_sse_timeseries_output(
...
function variable_movement_rate(;param_combinations = [(0.55, 32, 2, 5),
    ...
    results = pmap(x -> clean_vmr_timeseries_output(
                            runMANTIS(strainstructure=strainstruct,
                                      tmax=maxtime,
                                      tstep=trunc(Int, 0.9*maxtime):maxtime,
